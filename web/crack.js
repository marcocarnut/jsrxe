...
export async function runCrack({ plan, hash = "md5", targets, knobs = {}, onProgress, onHit, control }) {
  ...
  // Adaptive chunks, pipelined. Each prefix's inner sweep is sub-divided into
  // chunks aimed at ~CHUNK_SEC of GPU work; RING of them are submitted back to
  // back before a single drain, so the GPU never idles between chunks (draining
  // every chunk starved it and cost MD5 4x). A drain reads hits, reports, and
  // re-sizes the next chunks from the batch's measured rate -- so a fast hash
  // settles on large chunks and a slow one on small, both refreshing ~RING x
  // CHUNK_SEC apart, with a tiny first chunk so the first update is near-instant.
  const CHUNK_SEC = 0.1, CHUNK_MIN = 1 << 20, CHUNK_MAX = 1 << 30;
  let chunk = 1 << 22;
  let r = 0, inflight = 0, stopped = false, allFound = false, pausedMs = 0;
  let batchT0 = performance.now(), batchWork = 0;

  // Wait for the in-flight batch, stream hits, report, and steer the chunk size.
  // Returns true if the run should end (all found, or stopped).
  const drainReport = async () => {
    await device.queue.onSubmittedWorkDone();
    const dt = performance.now() - batchT0;
    await drainHits();
    if (foundHex.size >= ntgt) { allFound = true; return true; }
    if (dt > 0 && batchWork > 0)
      chunk = Math.max(CHUNK_MIN, Math.min(CHUNK_MAX, Math.round(batchWork / (dt/1000) * CHUNK_SEC)));
    if (onProgress) {
    ...
    if (control) {
      if (control.stopped()) { stopped = true; return true; }
    ...
    inflight = 0; batchWork = 0; batchT0 = performance.now();
    return false;
  };
