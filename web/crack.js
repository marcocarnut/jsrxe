...
function genericLane(G, w, idxExpr, H) {
  ...
  // The host-enumerated outer wheels are constant for the whole dispatch, so the
  // host pre-encodes them into message + plaintext word partials (P.pfxMsg /
  // P.pfxPt) once, and the kernel just loads them -- no per-candidate prefix loop.
  for (let k = 0; k < 16; k++) s += `    msg${w}[${k}] = P.pfxMsg[${k >> 2}u][${k & 3}u];\n`;
  for (let k = 0; k < 14; k++) s += `    pt${w}[${k}] = P.pfxPt[${k >> 2}u][${k & 3}u];\n`;
  s += `    var blen${w}: u32 = P.plen;\n`;
  s += `    var q${w} = ${idxExpr};\n`;
...
function genericShaderHead(H) {
  return `
struct Params { lo: u32, hi: u32, ntgt: u32, plen: u32, pfxMsg: array<vec4<u32>, 4>, pfxPt: array<vec4<u32>, 4> };
@group(0) @binding(0) var<storage, read>       targets : array<u32>;
@group(0) @binding(1) var<storage, read_write> hitcount: atomic<u32>;
@group(0) @binding(2) var<storage, read_write> hits    : array<u32>;
@group(0) @binding(3) var<uniform>             P       : Params;
...
export async function runCrack({ plan, hash = "md5", targets, knobs = {}, onProgress, onHit, control }) {
  ...
  const code = isPolicy
    ...
    : (mode === "serial" ? (inc ? serialShaderInc(A, wg, A.width, fw, H) : serialShader(A, wg, A.width, fw, H))
  ...
  // Uniform bytes: 4 scalars + the prefix region. The fast path stores raw prefix
  // bytes there (PFX_WORDS); the generic path stores TWO pre-encoded arrays --
  // pfxMsg and pfxPt -- so it needs 2x. Size for the larger; the fast path uses
  // only the first block.
  const U = 16 + 2 * PFX_WORDS * 4;
  ...
  const uBytes = new Uint8Array(ab, 16, PFX_WORDS * 4);          // fast path: raw prefix bytes
  const uMsg = new Uint32Array(ab, 16, PFX_WORDS);               // generic: pre-encoded message words
  const uPt = new Uint32Array(ab, 16 + PFX_WORDS * 4, PFX_WORDS);// generic: pre-encoded plaintext words
  const pfxScratch = new Uint8Array(PFX_WORDS * 4);
  ...
  const setPrefix = (oi) => {
    ...
    if (generic) {
      // Assemble the chosen outer members into a byte prefix, then pre-encode it
      // into the message + plaintext word partials the kernel loads (doing the
      // per-candidate prefix loop once, on the host).
      let plen = 0;
      for (let i = 0; i < outer.length; i++) {
        const p = outer[i], d = digits[i];
        if (p.L > 0) for (let k = 0; k < p.L; k++) pfxScratch[plen++] = p.bytes[d * p.L + k];
        else { const o = p.off[d], l = p.len[d]; for (let k = 0; k < l; k++) pfxScratch[plen++] = p.bytes[o + k]; }
      }
      uScal[3] = plen;                     // Params.plen
      uMsg.fill(0); uPt.fill(0);
      for (let a = 0; a < plen; a++) {
        const b = pfxScratch[a];
        uPt[a >> 2] |= b << ((a & 3) * 8);                         // plaintext, little-endian
        if (H.widen) { const mp = 2 * a; uMsg[mp >> 2] |= b << ((mp & 3) * 8); }
        else if (H.endian === "le") uMsg[a >> 2] |= b << ((a & 3) * 8);
        else uMsg[a >> 2] |= b << ((3 - (a & 3)) * 8);
      }
    } else {
      uBytes.fill(0);
      for (let i = 0; i < outer.length; i++) {
...
export async function runCrackCPU({ plan, hash = "md5", targets, onProgress, onHit, control }) {
  ...
  if (!fp.ok) return { supported: false, reason: fp.reason };
  // Even the oracle can't outrun physics: > 2^53 candidates is decades of CPU at
  // best, and past a JS number's exact range. Decline rather than spin forever.
  if (fp.total > BigInt(Number.MAX_SAFE_INTEGER))
    return { supported: false, reason: "over 2^53 candidates — too large to crack on the CPU" };
  const rows = parseTargets(targets, H);
  if (!rows.length) return { supported: true, empty: true };
