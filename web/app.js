...
async function renderCrack() {
  ...
  if (!navigator.gpu) {
    // No WebGPU: the GPU backend can't run, but the JIT'd multicore/CPU path can.
    // Make CPU the default, mark GPU unavailable, and keep cracking enabled.
    const be = $("knobbackend"), gpuOpt = be.querySelector('option[value="gpu"]');
    if (gpuOpt) gpuOpt.disabled = true;
    if (be.value === "gpu") be.value = "cpu";
  }
